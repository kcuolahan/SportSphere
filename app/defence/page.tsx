...
import { getTeamStyle, getCurrentPredictions } from "@/lib/data";
...
// ── Compute per-position ranks (1 = easiest for OVER bets = highest concession) ──
...
function rankBadge(rank: number) {
  ...
  return (
    <span style={{
      ...
      border: `1px solid ${color}40`, borderRadius: 3, padding: "1px 5px",
      marginLeft: 4,
    }}>#{rank}</span>
  );
}
...
function Cell({ pos, posKey, rank }: { pos: TeamPosition; posKey: string; rank: number }) {
  return (
    <td style={{
      padding: "11px 14px",
      background: heatColor(pos.vs_league, posKey),
...
// ── Heatmap view (one square per team × position) ─────────────────────────────
        ...
        <tbody>
          {teams.map(team => (
            <tr key={team.code} style={{ borderBottom: "1px solid #0a0a0a" }}>
              <td style={{ padding: "10px 14px", fontSize: 12, fontWeight: 700, color: "#f0f0f0", whiteSpace: "nowrap" }}>
                {team.name}
              </td>
              {positions.map(pos => {
                const posData = team.concedes_by_position[pos];
                const rank = RANKS[team.code]?.[pos] ?? 9;
                const bg = heatColor(posData.vs_league, pos);
                const isEasy = rank <= 6;
                const isHard = rank >= 13;
                return (
                  <td key={pos} style={{ padding: "10px 18px", background: bg, textAlign: "center" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: heatTextColor(posData.vs_league) }}>
                      #{rank}
                    </div>
                    <div style={{ fontSize: 9, color: "#555" }}>
                      {isEasy ? "Easy ▲" : isHard ? "Hard ▼" : "Neutral"}
                    </div>
                  </td>
                );
              })}
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: "#555" }}>{pick.team} vs {pick.opponent} · {pick.position}</div>
        </div>
...
function BestMatchupsPanel() {
  ...
  if (!dvpPlusPicks.length) {
    return (
      <div style={{ padding: "20px 0", fontSize: 13, color: "#555" }}>
        No picks this round combine top-6 DvP with the E/V filter.
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: "#000", color: "#f0f0f0", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "84px 20px 60px" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: "clamp(24px, 4vw, 40px)", fontWeight: 800, letterSpacing: "-0.03em", margin: "0 0 12px" }}>
            DvP Rankings
          </h1>
          <div style={{
            background: "#080808", border: "1px solid #111",
            borderRadius: 8, padding: "14px 16px", maxWidth: 680,
          }}>
            <p style={{ fontSize: 12, color: "#555", margin: 0, lineHeight: 1.7 }}>
              Teams in <span style={{ color: "#ef4444" }}>red concede more</span> than league average — good OVER targets.
              Teams in <span style={{ color: "#22c55e" }}>green concede fewer</span> — tilt toward UNDERs.
              Rank #1 = easiest matchup for that position (highest concession).
            </p>
          </div>
        </div>

        {/* Find Best Matchups button */}
        <div style={{ marginBottom: 24 }}>
          <button
            onClick={() => setShowBestMatchups(v => !v)}
            style={{
              background: showBestMatchups ? "#f97316" : "#0a0800",
              border: "1px solid #f97316",
              borderRadius: 8, padding: "10px 18px",
              fontSize: 12, fontWeight: 700,
              color: showBestMatchups ? "#000" : "#f97316",
              cursor: "pointer",
          ...
          >
            ...
            {dvpValueCount > 0 && (
              <span style={{
                fontSize: 10, background: "#f97316", color: "#000",
                borderRadius: 10, padding: "1px 7px", fontWeight: 800,
                display: showBestMatchups ? "none" : "inline",
              }}>{dvpValueCount}</span>
            )}
          </button>

          {showBestMatchups && (
            <div style={{ marginTop: 16, padding: 16, background: "#080808", border: "1px solid #1a1a1a", borderRadius: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {[
                { color: "rgba(239,68,68,0.28)", label: "OVER" },
                { color: "rgba(34,197,94,0.22)", label: "UNDER" },
              ].map(l => (
        ...
        <div style={{ marginTop: 16, fontSize: 11, color: "#555" }}>
          Data based on {TEAMS[0]?.concedes_by_position.MID.games ?? 0} rounds sampled. Rank #1–6 = easy matchup (green). Rank #13–18 = hard (red). Analytics only — not betting advice.
        </div>
      </div>
